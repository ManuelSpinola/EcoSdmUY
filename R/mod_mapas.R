...
# Mapa vacío centrado en Uruguay
.mapa_vacio <- function() {
  leaflet::leaflet() |>
  ...
  if (tipo == "presente") {
    div(class = "p-3",
      ...
      layout_columns(col_widths = c(6, 6),
        ...
        card(card_header(bs_icon("layers", class = "me-1"), "Categorías de hábitat"),
      ...
      layout_columns(col_widths = c(3, 3),
        ...
        downloadButton(ns("dl_cat"),  "Descargar categórico (.gpkg)", class = "btn-outline-secondary btn-sm w-100")
      )
    )
    ...
    div(class = "p-3",
      ...
      layout_columns(col_widths = c(6, 6),
        card(card_header(bs_icon("palette", class = "me-1"), "Idoneidad futura continua (0-1)"),
             card_body(class = "p-0", leaflet::leafletOutput(ns("mapa_cont"), height = "520px"))),
        card(card_header(bs_icon("layers", class = "me-1"), "Categorías de hábitat futuro"),
             card_body(class = "p-0", leaflet::leafletOutput(ns("mapa_cat"), height = "520px")))
      ), br(),
      layout_columns(col_widths = c(3, 3),
        downloadButton(ns("dl_cont"), "Descargar continuo (.gpkg)", class = "btn-outline-primary btn-sm w-100"),
    ...
    div(class = "p-3",
      uiOutput(ns("resumen")), br(),
      layout_columns(col_widths = c(6, 6),
        card(card_header(bs_icon("shield-check", class = "me-1"), paste0("Área de Aplicabilidad (AOA)", sufijo)),
             card_body(class = "p-0", leaflet::leafletOutput(ns("mapa_aoa_bin"), height = "480px"))),
        card(card_header(bs_icon("graph-up", class = "me-1"), paste0("Índice de Disimilaridad (DI)", sufijo)),
      ...
      layout_columns(col_widths = c(6, 6),
        ...
        card(card_header(bs_icon("layers", class = "me-1"), paste0("Categorías dentro del AOA", sufijo)),
...
mod_mapas_server <- function(id, estado, tipo = "presente") {
  moduleServer(id, function(input, output, session) {
    ...
    if (tipo %in% c("presente", "futuro")) {
      ...
      titulo_cat  <- if (tipo == "presente") "Hábitat presente"   else "Hábitat 2061-2080"
      ...
      output$resumen <- renderUI({
        ...
        if (is.null(p)) {
          return(div(class = "alert alert-light small py-2 px-3",
            ...
            if (tipo == "presente") "Presioná 'Ver distribución' para generar el mapa."
      ...
      msg_espera <- if (tipo == "aoa_futuro")
        "El AOA futuro se calcula junto con la distribución futura."
      else
        "El AOA se calcula automáticamente junto con la predicción presente."
      ...
      output$resumen <- renderUI({
        ...
        div(class = "alert alert-info small py-2 px-3",
          ...
          strong(nrow(r)), " hexágonos | Dentro del AOA: ", strong(n_in),
      ...
      output$mapa_aoa_bin <- leaflet::renderLeaflet({
        ...
        pal_aoa <- leaflet::colorFactor(palette = c("#d9d9d9", "#0038A8"),
        ...
        leaflet::leaflet() |>
          ...
          leaflet::addLegend(position = "bottomright",
            colors = c("#0038A8", "#d9d9d9"), labels = c("Dentro del AOA", "Fuera del AOA"),
